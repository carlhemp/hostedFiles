...
function drawViz(data) {
  ...
  //Concat the 2nd dimension
  let concatTable = {};
  for(let j = 0; j < data.tables.DEFAULT.rows.length; j++) {
    let row = data.tables.DEFAULT.rows[j];
    if(row[0] == null) {
      continue;
    }
    let concatRow = concatTable[row[0]] || null;
    if(!concatRow) {
      concatRow = row;
    }
    else {
    ...
    concatTable[row[0]] = concatRow;
  }

  //Format the concatenated table
  Object.values(concatTable).forEach(function(row) {
